...
#import "UpEventViewController.h"

@interface UpEventViewController ()

@end

@implementation UpEventViewController
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(startSearch:)
                                                 name:NOTIFICATION_SEARCH_EVENT
                                               object:nil];
}
...
- (void)searchEventsByKeyword:(NSString*)keyword{
    ...
    NSString *url = [NSString stringWithFormat:@"%@%@?own=1&after=%@&keyword2=%@", BASE_URL, EVENT_SEARCH, [Functions getCurrentDateTime], keyword];
    url = [url stringByReplacingOccurrencesOfString:@" " withString:@"%20"];
...
{
    if ([eventArray count] > 0 && !_whileSearch) { //disable pagination while search
